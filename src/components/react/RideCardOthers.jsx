...
const RideCardOthers = ({ allRides }) => {
    ...
    const onJoinClick = async (rideId, to, us) => {
        ...
        try{
            const response = await fetch(import.meta.env.PUBLIC_BACKEND_URL+'/api/rides/join-ride', {
                method: 'POST',
            ...
            if(data.success)
            {
                    setStatus((prev) => ({
                        ...prev,
                        [rideId]: data.message || "Ride Joined Successfully",
                    }));
                    setTimeout(() => {
                    ...
                    setStatus((prev) => ({
                        ...prev,
                        [rideId]: data.message || "Failed to Join Ride",
                    }));
        ...
        >
          ...
          <a href={`/ride/${ri._id}`} className="block hover:no-underline text-black">
            ...
            <div className="p-5 flex-grow">
              ...
              <p className="mb-3 text-sm tracking-tight text-gray-700 line-clamp-4">
                {ri.description}
              </p>
            </div>
          </a>
          ...
          <div className="ride-card p-5">
            <div onClick={()=>onJoinClick(ri._id)}
              className="join-btn mt-auto inline-flex items-center py-2 font-semibold text-center cursor-pointer"
              data-ride-id={ri._id}
            >
              Join Ride
            </div>
